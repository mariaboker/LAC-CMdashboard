...
# Load data
df = pd.read_csv("disease_case_dashboard_data.csv", parse_dates=["Date Opened"])
...
with fcol2:
    selected_supervisor = st.selectbox("Select Supervisor", ["All"] + sorted(df["Supervisor Name"].unique()))
with fcol3:
    selected_investigator = st.selectbox("Select Investigator", ["All"] + sorted(df["Investigator Name"].unique()))
...
# Define active case statuses
active_statuses = [
    "Open", "New Supervisor (assigned)", "New PHN (assigned)", "Returned (to Program)",
    "Returned (by Supervisor)", "Returned (by Investigator)", "To AMD (under review)",
...
# Summary metrics with bigger text
...
with metric_col2:
    ...
    st.metric("Cases Needing Supervisor Action", len(filtered_df[filtered_df["Status"].isin(action_statuses)]))
with metric_col3:
    st.metric("Currently with PHI", len(active_cases[active_cases["Status"] == "PHI Support Requested"]))

# Status chart
status_order = [
    "New Supervisor (assigned)", "New PHN (assigned)", "Open", "Reassign",
    "Returned (by Investigator)", "Returned (by Supervisor)", "To AMD (under review)",
    "To Supervisor (under review)", "Returned (by AMD)", "Approved (by AMD)"
]
...
# Toggle filter for table
st.markdown("### Filtered Case Table")
table_filter_option = st.radio("Show:", ["All Cases", "Cases Needing Supervisor Action"])

if table_filter_option == "Cases Needing Supervisor Action":
    table_df = filtered_df[filtered_df["Status"].isin(action_statuses)]
else:
    table_df = filtered_df

# Reorder columns to place Status right after Case ID
...
# Add line to separate table from charts
st.markdown("---")

# Charts for Program and SPA
